...
let workerQueue = dispatch_queue_create("com.ray.worker", attr)
...
func doComplexWork() {
  sleep(1)
    print("\(stopClock()) :: \(currentQueueName())")
}
...
let concurrentQueue = dispatch_queue_create("com.ray.concurrent", DISPATCH_QUEUE_CONCURRENT)
